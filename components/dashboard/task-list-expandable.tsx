      ...
      {tasks.map((task) => (
        <li key={task.id} className="rounded-md border border-slate-200 bg-slate-50 px-3 py-2">
          ...
          <p className="mt-1 text-xs text-slate-600">
            {task.categoryName} — {task.columnName || (task.status === "finalizada" ? "Finalizado" : task.status === "emAndamento" ? "Em andamento" : "Backlog")} — {task.completedAt ? new Date(task.completedAt).toLocaleDateString("pt-BR") : "sem data"}
          </p>
        </li>
      ))}
    </ul>
