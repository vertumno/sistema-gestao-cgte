...
describe("metrics", () => {
  ...
  it("aggregates categories with zero values for empty categories", () => {
    ...
    expect(result.categories.find((item) => item.categoryId === 1)?.finalizadas).toBe(1);
  ...
  it("filters tasks by area", () => {
    const input = [
      { categoryId: 1, area: "Design" },
      { categoryId: 12, area: "Libras" }
    ] as const;
    ...
    expect(filtered[0].categoryId).toBe(12);
  });
});
